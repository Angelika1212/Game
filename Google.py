...
class DATA_BASE():
    ...
    def add_to_date_base(self, inform):
        ...
        if magnetic_field == 'strong' or  magnetic_field == 'the strongest':
            ...
            if (temperature == 300 or temperature == 0) and oxygen == 0:
                if speed_of_wind == 0 or speed_of_wind > 20 and water < 10:
                    if magnetic_field == 'weak':
                        DATA_BASE().add_to_date_base('Самые неблагоприятные условия')
            if 90 <= oxygen <= 100:
                DATA_BASE().add_to_date_base('Кислородная катастрофа')                
...
def cheek_temperature():
    ...
    if temperature == 0:
        DATA_BASE().add_to_date_base('Ледниковый период')
    if temperature == 300:
        DATA_BASE().add_to_date_base('Огненная земля')        
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.USEREVENT:
            ...
            if pygame.draw.rect(screen, (255, 255, 255), (50, 50, 50, 60), 3).collidepoint(event.pos):
                game_event = 'start_game'
